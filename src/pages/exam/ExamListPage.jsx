        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {isLoading
            ? [1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                <div
                  key={i}
                  className="h-[240px] animate-pulse rounded-[2rem] bg-gray-100 dark:bg-gray-800"
                />
              ))
            : exams.map((exam) => {
                const isStartingThisExam = startingExamId === exam.id;
                ...
                return (
                  ...
                  <Link
                    ...
                    className="group relative flex h-[240px] w-full flex-col items-center justify-center overflow-hidden rounded-[2rem] border border-gray-100 bg-white p-5 text-center transition-all duration-300 hover:-translate-y-2 hover:border-primary/50 hover:shadow-xl dark:border-gray-800 dark:bg-gray-900"
                  >
                    ...
                    <div className="relative z-10 mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gray-900 text-xl font-black text-white transition-colors duration-300 group-hover:bg-primary dark:bg-black">
                      {exam.level}
                    </div>

                    <h2 className="relative z-10 px-2 text-base font-bold text-gray-800 line-clamp-2 transition-colors group-hover:text-primary dark:text-white">
                      {exam.title}
                    </h2>
                    ...
                    <div className="absolute bottom-4 flex translate-y-2 items-center gap-1 text-[10px] font-black uppercase tracking-tighter text-primary opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                      Xem chi tiết bài thi <ChevronRight size={12} />
                    </div>
